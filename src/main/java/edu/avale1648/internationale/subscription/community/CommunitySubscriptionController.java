...
public class CommunitySubscriptionController {
	...
	@PutMapping("/{id}")
	public ResponseEntity<CommunitySubscription> update(@PathVariable Long id, @RequestBody CommunitySubscription subscribtion) {
		CommunitySubscription current = repository.findById(id).orElseThrow(RuntimeException::new);
		current = repository.save(new CommunitySubscription(subscribtion));
